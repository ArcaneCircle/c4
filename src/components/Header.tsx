...
const Header = (props: HeaderProps) => {
  ...
  return (
    <header className={props.blur ? "blur" : ""}>
      ...
      <h2 className={isFirstPlayerTurn ? 'red' : 'yellow'}>{isFirstPlayerTurn ? players[0].addr === playerAddr ? 'Your' : players[0].name + '\'s' : players[1].addr === playerAddr ? 'Your' : players[1].name + '\'s'} turn</h2>
    </header>
  )
}
