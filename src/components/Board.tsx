      ...
      {({
        ...
        return (
          <>
            ...
            <section className={game.state.winner ? "board blur" : "board"}>
              {gameArray.map((cell, index) => {
                return (
                  <Ball
                    ...
                    onClick={() => {
                      ...
                      const isTurn = isPlaying
                        ? game.state.playing.color ===
                          players.find((p) => p.addr === playerAddr)?.color
                        : false;
                      ...
                      if (isTurn && game.insert(col)) {
                        ...
                        const text = game.state.winner
                          ? playerName + " won!"
                          : nextPlayer?.name + "'s turn";
                        window.webxdc.sendUpdate(
                          {
                            payload: { move: col, moves: newMoves, players },
                            summary: text,
                            notify: {
                              [nextPlayer?.addr]: game.state.winner
                                ? text
                                : "It's your turn",
                            },
                          },
                  ...
                  >
                    {game.state.board[index] === null ? (
                      <div></div>
                    ) : (
                      <div
                        className={
                          game.state.board[index].color === "#000"
                            ? index === lastMoveIndex
                              ? "dropdown player1"
                              : "player1"
                            : index === lastMoveIndex
            ...
            <Status
              blur={!!game.state.winner}
              color={game.state.playing.color}
            />
            {game.state.winner && (
              <WinnerScreen winner={game.state.winner} players={players} />
            )}
          </>
        );
      }}
